export const formatNumberToTime = (number: number) => {
  const numToDate = new Date(number * 1000);
  const hours = String(numToDate.getHours()).padStart(2, "0");
  const minutes = String(numToDate.getHours()).padStart(2, "0");

  return { hours, hoursAndMinutes: hours + ":" + minutes };
};
...
export const getPressure = (value: number) =>
  value < 1013 ? "Lower" : "Heigher";

export const getFeelsLike = (
  feelsTemeprature: number,
  currentTemeprature: number
) => (feelsTemeprature < currentTemeprature ? "colder" : "warmer");
